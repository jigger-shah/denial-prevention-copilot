...
def _getting_started_dialog() -> None:
    ...
    with st.expander("What Changed (Release Notes)"):
        st.markdown(
            ...
            "ephemeral. Follow-up: fixed a schema-initialization bug where a "
            "long-running process could hit \"no such table: audit_decisions\" if "
            "the OS cleared the temp DB file mid-session — the audit schema now "
