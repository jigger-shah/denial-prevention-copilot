"""
Coding Validation Agent — v1 (reuses Coverage Agent's retrieval, different reasoning).

Retrieves up to 3 LCD/NCD/Article policy chunks for the claim's CPT and
ICD-10 codes (same source material as the Coverage Agent — retrieval order
and corpus are unchanged, see _retrieve_policies()), then asks Claude (via
structured tool use) to evaluate coding defensibility: diagnosis specificity,
diagnosis-to-procedure support, and payer scrutiny risk. The prompt — not a
different retrieval path — is what separates this agent from Coverage
Validation.

This agent must not duplicate the deterministic rule layer (NCCI, MUE,
modifiers, CPT/ICD validity) — those checks already ran in rules/ before the
orchestrator ever calls this module. The system prompt instructs the model to
assume those checks are already done and to focus only on reasoning the rule
layer cannot perform.

Governance rules (all enforced here, identical to Coverage Validation):
  - No API key → return []
...
_SYSTEM_PROMPT = (
    "You are a medical coding compliance specialist reviewing a healthcare claim "
    "for coding defensibility issues before submission.\n\n"
    ...
    "diagnosis and procedure codes on this claim are well-supported and defensible under "
    "payer scrutiny, based solely on the provided policy text.\n\n"
    "Do not identify NCCI edits, MUE violations, modifier requirements, code validity issues, "
    ...
    "- If the diagnosis and procedure codes are well-supported by the policy text, "
...
_TOOLS = [
    {
        "name": "report_coding_finding",
        "description": (
            "Report a coding defensibility concern identified in the claim. "
        ...
        "input_schema": {
            ...
            "properties": {
                "issue": {
                    "type": "string",
                    "description": "Short description of the coding defensibility concern.",
                },
                "recommendation": {
                    "type": "string",
                ...
                "confidence": {
                    "type": "number",
                    "description": "Confidence score 0.0–1.0 that this is a real coding concern.",
                },
                "citation_doc_id": {
                    "type": "string",
    ...
    {
        "name": "no_coding_concern",
        "description": (
            "Call this tool when the diagnosis and procedure codes are well-supported "
            "by the provided policies and you have no coding defensibility concern to report."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
...
def validate_coding(
    ...
    try:
        ...
        logger.warning("Coding validation error: %s", exc)
        return [], lcd_policies
...
def _parse_response(
    ...
    for block in response.content:
        ...
        if block.name == "no_coding_concern":
            return []

        if block.name == "report_coding_finding":
            args = block.input
            ...
            if doc_id not in retrieved_doc_ids:
                logger.warning(
                    "Coding agent cited doc_id %r not in retrieved set %r — suppressed",
                    doc_id,
                    retrieved_doc_ids,
                )
                return []
            ...
            finding = Finding(
                ...
                severity=severity,
                issue=args.get("issue", "Coding defensibility concern identified"),
                recommendation=args.get("recommendation", "Review coding and documentation."),
                citation=citation,
...
def _stable_finding_id(claim_id: str, doc_id: str, issue: str) -> str:
    ...
    return "cod-" + hashlib.sha256(payload.encode()).hexdigest()[:12]
