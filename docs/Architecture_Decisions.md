...
> **Update (v1.9 follow-up, schema initialization on every connect, post-2026-06-21):** The temp-dir default above has a consequence this ADR didn't originally account for: the OS can delete a temp-dir file out from under a still-running process, and `app/main.py:get_repo()` only calls `initialize_database()` once per server process (it's `@st.cache_resource`). A deleted-then-silently-recreated-empty SQLite file left a cached `AuditRepository` pointed at a file with no `audit_decisions` table, surfacing as `sqlite3.OperationalError: no such table: audit_decisions` on every subsequent read. Fixed by moving schema creation (the `CREATE TABLE IF NOT EXISTS` plus both backward-compatible `ALTER TABLE` migrations) into `_connect()` itself, so it is re-applied — idempotently, at negligible cost — on every connection rather than only at startup; `initialize_database()` is now a thin wrapper around `_connect()`. This makes the repository self-healing against the ephemeral-filesystem behavior this ADR's temp-dir decision deliberately introduced. See `docs/Repository_Status_Report.md`'s v1.9 follow-up update.
