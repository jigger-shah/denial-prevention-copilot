...
> **Update (CMS Data Control UI, post-2026-06-22):** The Data Source Status pill's check is now fully user-initiated, mirroring the gear-icon session API key control's UX philosophy (optional, user-initiated, hidden behind a click, never automatic). The header no longer calls `rules.data_source_status` (and therefore never attempts a CMS asset download) on page load — the pill defaults to a neutral "⚪ Data: Not Refreshed," and only resolves to "🟢 Live CMS"/"🟡 Synthetic fallback"/"🟡 Mixed" once the user clicks "🔍 Check CMS Data Availability" inside the same popover the pill already used. A "🔄 Refresh Data Status" button (useful after updating secrets or deploying new CMS assets) clears both `_data_source_summary()`'s own cache and the deeper per-loader/per-asset caches (`ncci_loader`, `mue_loader`, `icd10_loader`, `cms_asset_fetch`) and re-checks immediately. No change to how status is calculated, how CMS assets are downloaded, or to the AI key gear control. 11 new tests (`tests/test_cms_data_control.py`), 2 updated for the new on-demand behavior (`tests/test_cms_asset_fetch.py`'s boot tests). See `README.md`'s "Data Source Status pill" and "Live CMS Data" sections.
>
> **Update (Phase 13, Session-Scoped Anthropic API Key, post-2026-06-22):** Any user — not just the app owner — can now provide their own `ANTHROPIC_API_KEY` for live AI via a small ⚙️ gear-icon popover next to the AI status pill, scoped to their browser session only (`st.session_state`, never persisted to disk, never logged, never written to the audit trail). `agents/secrets.py:get_secret()` now checks the session key first, then the existing env-var/`st.secrets` order, unchanged for app-owner keys; `agents/orchestrator.py`, `agents/coverage_validation.py`, and `agents/coding_validation.py` needed no changes since all three already resolved their key through `get_secret()`. The AI pill's wording is unchanged ("AI: Enabled"/"AI: Disabled" only); the old `.env`-instructing banner was removed from the main UI entirely. 15 new tests, 564 passing (up from 549). See `docs/Roadmap.md` Phase 13 and `README.md`'s "AI features" section.
>
> **Update (Phase 12 Deferred, post-2026-06-22):** A planning analysis for CMS Policy Retrieval Enhancement (Chroma + LCD/NCD assets) was completed and the work was **deferred** — tracked as `docs/Technical_Debt_Register.md` TD-28. The analysis confirmed the real LCD/NCD assets (`LCD_33431.json`, `NCD_98.json`) index successfully with no transformation needed, and identified a prebuilt-Chroma-index-as-GitHub-Release-Asset architecture as the preferred future direction — but implementation was not started. Deferred because it's not required for MVP V1 or for Live CMS rule-layer mode (Phase 11), and because the embedding-model download dependency, Streamlit Cloud memory/latency profile, and the small 15-claim golden set all need attention before this is worth the operational risk. No code changed — Coverage/Coding retrieval remains exactly as Phase 11 left it (Chroma-first when available, JSON fallback otherwise). See `docs/Roadmap.md` Phase 12.
>
> **Update (Phase 11, Live CMS Rule-Layer Mode, post-2026-06-22):** Hosted deployments can now optionally use the real NCCI/MUE/ICD-10 reference datasets instead of the synthetic fallback tables, without committing the ~266MB files to the repo (closing the "remaining gap" left open by TD-27 — see `docs/Technical_Debt_Register.md`). New `rules/cms_asset_fetch.py` lazily downloads from maintainer-configured GitHub Release Asset URLs (6 independent, optional env vars — no defaults, no hardcoded URLs) into a `tempfile.gettempdir()`-based cache, attempted at most once per process, atomic-write, never raising — every failure mode degrades to the existing local-file-or-synthetic-fallback behavior unchanged. `rules/ncci_loader.py`/`mue_loader.py`/`icd10_loader.py` resolve their reference directory lazily (cache dir first, then local `data/reference/`, unchanged) only on the `reference_dir=None` default path — every existing call site and test that passes an explicit `reference_dir` is unaffected. `rules/data_source_status.py` gained additive `source`/`download_attempted`/`download_error` fields for an honest "Mixed"/"download failed, using fallback" UI popover; the existing `file_backed`/`synthetic_fallback`/`mixed` pill logic is unchanged. **Scope:** rule layer only (NCCI, MUE, ICD-10) — Chroma indexing, LCD/NCD policy retrieval, and Coverage/Coding agent behavior are explicitly untouched. 26 new tests (`tests/test_cms_asset_fetch.py` + additions to `tests/test_data_source_status.py`). 549 tests passing (up from 523). See `docs/Roadmap.md` Phase 11 and `README.md`'s "Live CMS Data" section.
>
> **Update (Public Release Milestone, post-2026-06-22):** The project is now designated **V1.0 MVP**. This milestone reflects completion of the MVP scope described in the "Update (MVP V1, post-2026-06-21)" entry directly below — same engineering substance, restated as the public-facing product version rather than an internal engineering label. Subsequent v1.9.x releases (tagged `v1.9.1-streamlit-cloud-fix` — ChromaDB/Streamlit Cloud Python-3.14 import compatibility — plus an untagged follow-up commit fixing manual-claim AI execution parity and resilience) document deployment readiness, cloud compatibility, reliability improvements, and operational hardening — they extend V1.0 MVP, they do not redefine it. 523 tests passing (up from 500). See `README.md`'s "Current Public Release" section and `docs/Roadmap.md`'s public-release note.
>
> **Update (MVP V1, post-2026-06-21):** MVP V1 is complete as of v1.9 — this is the current-state summary; everything below is detailed engineering history leading up to it. MVP V1 was implemented through three releases: v1.8a (UI polish and first-time UX), v1.8b (citation transparency / Supporting Policies Reviewed), and v1.9 (Streamlit Cloud deployment readiness + audit schema self-healing fix). **Product capabilities:** deterministic Rule Engine (NPI, NCCI, MUE, ICD-10, HCPCS, modifier, diagnosis-procedure checks), Coverage Validation Agent, Coding Validation Agent, Unified Risk Assessment, citation grounding (no citation → no finding), Supporting Policies Reviewed, human Accept/Override review workflow, and an append-only audit trail. **Deployment readiness:** `agents/secrets.py:get_secret()` resolves `ANTHROPIC_API_KEY`/`ANTHROPIC_MODEL` from either a local `.env` or Streamlit Cloud's `st.secrets`; the audit database defaults to the OS temp directory for Streamlit Cloud's ephemeral filesystem. **Fresh clone validation:** confirmed PASS — no `.env`, no API key, no local CMS datasets, no seeded Chroma store, no pre-existing `audit.db`, fresh venv from pinned `requirements.txt`, app boots and runs end-to-end. **Audit schema self-healing:** `db/audit_repository.py:_connect()` re-applies the `CREATE TABLE IF NOT EXISTS` schema and migrations on every connection (not just at startup), so a missing or recreated-empty DB file self-initializes on first use, closing a bug where a long-running cached `AuditRepository` could hit `sqlite3.OperationalError: no such table: audit_decisions`. **500 tests passing.** See `docs/Roadmap.md`'s "MVP V1 Achieved" milestone section and `README.md`'s "Current Product Release" line.
>
...
>
> **Update (v1.7, Quality Hardening Sprint, post-2026-06-19):** A small, deliberately scoped set of TD items closed without growing the agent count, building a full HCPCS reference-file platform, or touching UI/deployment. The "TD-06 | Only 2 hardcoded code validity rules" row below (Section 6) and the "ICD-10-CM code validity"-adjacent gaps are now stale: `rules/hcpcs.py` adds a curated ~16-code HCPCS Level II format-recognition check (not a full file-backed loader — see TD-06), and `rules/code_validity.py` gained two new modifier rules (`missing_modifier_76` repeat-procedure, `missing_modifier_50` bilateral), bringing total active modifier rules to 3. The "TD-15 | Citation edition says 'synthetic sample'" row is resolved: `code_validity.py`'s citation `edition` strings now read `"(curated interpretive rule — not file-backed)"`. TD-20 (citation excerpts never persisted) is resolved: `db/audit_repository.py` gained a `citation_excerpt` column via a backward-compatible migration, threaded through to `app/main.py`'s Save Decision handler. TD-24 is now **partially resolved** across four passes within this sprint: Phase 1 (golden-set label review — relabeled `GOLD-008`/`GOLD-009` as legitimate agent-positive claims, added `GOLD-015` as an explicit agent-negative baseline), Phase 2 (live re-measurement confirming the precision improvement), Phase 3 (anti-pile-on prompt calibration — overshot, recall dropped), and Phase 3B (`rule_findings`-conditional narrowing — recovered recall while keeping the precision gains; current live benchmark: Overall 0.92 precision / 0.85 recall / 0.88 F1, Coverage Agent 1.00/0.50/0.67, Coding Agent 0.60/0.60/0.60). A follow-up live Haiku-vs-Sonnet-vs-Opus benchmark on the same Phase 3B prompts found Sonnet 4.6 had the best overall F1 — `_DEFAULT_MODEL` in both agent modules was changed from `claude-haiku-4-5` to `claude-sonnet-4-6` on that basis (override via `ANTHROPIC_MODEL`; Haiku/Opus remain supported). TD-27 gained a backend-only real-data-vs-synthetic-fallback status check (`rules/data_source_status.py`); no UI indicator was added, by design. See `docs/Roadmap.md` Phase 8.7 and `docs/Technical_Debt_Register.md` for full detail. 481 tests passing (up from 426). The readiness scores in Section 7 below predate this sprint and were not revised — this sprint closed a handful of TD items, not the broader feature-maturity gaps (ChromaDB pre-seeding, Documentation Review Agent, deployment) those scores measure.
>
> **Update (v1.8a, UI Polish & First-Time User Experience, post-2026-06-20):** This sprint touched the UI exclusively — no agent, prompt, or rule-layer logic changed except a bounded TD-25 investigation that was reverted (see below). The sidebar referenced throughout this report's "Current Architecture" and "Implemented Features" sections (Sections 2-3) no longer exists: reviewer name entry and AI status moved to a header bar, alongside a new **Data Source Status pill** ("Data: Live CMS" / "Data: Synthetic fallback" / "Data: Mixed") that surfaces TD-27's v1.7 backend status check in the UI for the first time — closing the UI half of TD-27. The "Sample Claim mode" / "Manual Claim Entry mode" two-tab split described in Section 2's architecture diagram and Section 3's feature list is also stale: claim entry is now one "Claim Details" flow with a "Pick a demo scenario" / "Enter manually" picker, a landing state with recommended demo shortcuts, and demo-type tags. A "❔ Getting Started" onboarding dialog was added (how-it-works, architecture diagram, AI/demo-mode legend, confidence legend, limitations, release history). Finding cards gained a source badge (Rule Engine / Coverage Agent / Coding Agent) and a plain-English "why this matters" line; the risk banner gained a severity-count breakdown and recommended action. Two real bugs were found and fixed: a Streamlit frontend quirk where "Clear Form" cleared `session_state` but not the displayed field values (fixed via versioned widget keys), and "Add Service Line"/row-remove requiring two clicks because neither called `st.rerun()`. TD-25 was re-investigated (widening `dx_procedure_conflict` to the full Z00 prefix family) but reverted after it regressed two calibrated golden-set tests — remains deferred. See `docs/Roadmap.md` Phase 8.8 and `docs/Technical_Debt_Register.md` TD-27 for full detail. 482 tests passing (up from 481). The readiness scores in Section 7 below predate this sprint and were not revised — this sprint was UI/UX, not the underlying feature-maturity gaps those scores measure.
>
> **Update (v1.8b, Citation Transparency, post-2026-06-20):** Closes TD-22 (retrieved policy transparency limited to one displayed citation), referenced as open in Section 6's Technical Debt Assessment below. `agents/coverage_validation.py:validate_coverage()` and `agents/coding_validation.py:validate_coding()` now return `(findings, retrieved_policies)` instead of just `findings` — the up-to-3 policies each agent already retrieves internally, no longer discarded after the model call. `agents/orchestrator.py:run_review()` returns `(RiskAssessment, retrieved_policies)` as a sibling tuple; `RiskAssessment`'s own shape and the audit/DB layer are unchanged. The UI gained a "📚 Supporting Policies Reviewed" expander on AI-sourced finding cards, showing other retrieved-but-uncited policies, explicitly captioned as considered-but-not-the-basis-for-this-finding. 6 new tests assert the new return shape; a real bug in `evaluation/harness.py`'s offline-mode mock (returning a bare `[]` that would have broken on the new tuple unpacking) was caught and fixed in the same pass. See `docs/Roadmap.md` Phase 8.9 and `docs/Technical_Debt_Register.md` TD-22 for full detail. 488 tests passing (up from 482).
>
> **Update (Phase 10 code readiness, pre-v1.9, post-2026-06-20):** Streamlit Cloud deployment has not happened yet, but the known code-level blockers are closed. New `agents/secrets.py:get_secret(name, default="")` resolves a secret from the OS environment variable first (so local `.env`/python-dotenv behavior is completely unchanged), falling back to `st.secrets` only when the environment variable isn't set — wrapped in `try/except` so a local run with no `.streamlit/secrets.toml` never crashes. Replaces every functional `os.getenv("ANTHROPIC_API_KEY")` / `os.getenv("ANTHROPIC_MODEL", ...)` call in `app/main.py`, `agents/orchestrator.py`, `agents/coverage_validation.py`, and `agents/coding_validation.py`. Separately, `db/audit_repository.py:DB_PATH` now defaults to `pathlib.Path(tempfile.gettempdir()) / "denial_copilot_audit.db"` instead of a path inside the repo, since Streamlit Cloud's filesystem is ephemeral; `AuditRepository`'s explicit `db_path` override (used by every test) is unaffected. README gained one sentence documenting that the audit trail is demo-local and may reset on hosted restarts/redeploys/sleep — acceptable since the app uses synthetic data only. Still open per `docs/Roadmap.md` Phase 10: `requirements.txt` verification against an actual Streamlit Cloud build, configuring the real Cloud secret, and the deploy itself. 9 new tests (`tests/test_secrets.py` + 2 in `tests/test_audit.py`). 497 tests passing (up from 488).
>
> **Update (fresh-clone validation, post-2026-06-20):** Cloned the repo into an isolated directory with no `.env`, no `ANTHROPIC_API_KEY`, no local CMS reference files, no seeded Chroma store, and no pre-existing `audit.db`, then built a fresh venv from the pinned `requirements.txt` and ran the app end-to-end — confirmed PASS with no blockers (see `docs/Roadmap.md` Phase 10). The only finding was cosmetic: a harmless Streamlit session-state warning logged (never user-visible) when "Start from a template" populated the service-line `Units` field, caused by a redundant `value=1` alongside an already-equivalent `min_value=1`. Removed the redundant default in `app/main.py` — confirmed behaviorally identical (fresh rows, template-loaded rows, and "Add Service Line" all still default to `1`) via live verification, full suite still 497 passing.
>
> **Update (v1.9 follow-up, audit DB schema initialization fix, post-2026-06-21):** A long-running local Streamlit process hit `sqlite3.OperationalError: no such table: audit_decisions` on the Audit Trail tab. Root cause: `get_repo()` (`app/main.py`) is `@st.cache_resource`, so `AuditRepository.initialize_database()` only ran once per server process; the v1.9 `DB_PATH` default lives in the OS temp directory, which the OS can clear out from under a still-running process, and the next `sqlite3.connect()` silently recreates an empty file with no tables. Fix: `db/audit_repository.py:_connect()` now (re)applies the `CREATE TABLE IF NOT EXISTS` schema and both backward-compatible `ALTER TABLE` migrations on every connection, not just at startup — `initialize_database()` is now a thin wrapper around `_connect()`. `get_decisions()`, `export_decisions_csv()`, and `save_decision()` all route through `_connect()`, so a brand-new or schema-missing DB file self-initializes on first use of any of them, including reads. 3 new regression tests in `tests/test_audit.py` cover: a DB file that exists with no `audit_decisions` table for `get_decisions()`, `export_decisions_csv()`, and `save_decision()` — each must succeed instead of raising. 500 tests passing (up from 497). No commits made; changes are local pending review per explicit instruction.
