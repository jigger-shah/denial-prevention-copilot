...
**Remaining gap:** No deploy-time fetch step (e.g. auto-downloading the real CMS files on hosted deployment). Not currently planned — the UI indicator's job is to make the situation visible and honest, not to eliminate it.

**Status:** Resolved (backend v1.7, UI v1.8a). Identified during Deployment Readiness Review (June 2026); the programmatic status check is now surfaced directly to reviewers via the header pill.
