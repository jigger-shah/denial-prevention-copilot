...
- ✅ ChromaDB made optional at runtime; hosted demo falls back to the JSON policy corpus if the vector store is unavailable. The first actual Streamlit Cloud deploy attempt failed at app startup: `chromadb`'s import chain (`chromadb` → `opentelemetry` → generated `protobuf` `_pb2` modules) raised a bare `TypeError` on Cloud's Python 3.14 runtime, before any of the existing vector-store-then-JSON-fallback retrieval logic could run — a module-import-time failure, not a runtime call failure, so the existing `try/except` in `_retrieve_from_vector_store()` never got a chance to catch it. `retrieval/vector_store.py` now wraps `import chromadb` itself in a broad `except Exception` (not just `ImportError`, since the observed failure is a `TypeError`), so the module stays importable either way; `VectorStore.__init__` raises a clear `RuntimeError` (chaining the original import error) when chromadb is unavailable, which the agents' existing fallback already catches. Added `runtime.txt` pinning `python-3.13` (matching local dev) to avoid Cloud defaulting to an untested newer runtime. 8 new tests (`tests/test_vector_store.py`, `tests/test_coverage_validation.py`, `tests/test_coding_validation.py`) cover both the import-failure and construction-failure paths.
...
- ⬜ Streamlit Cloud secrets configured for `ANTHROPIC_API_KEY` — deploy-time action, not yet performed (no successful deploy attempted)
